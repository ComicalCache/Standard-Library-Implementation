...
namespace ext {
	...
	private:
		 ...
		 */
		bool internal_swap_on_demand(size_t pos1, size_t pos2) {
			if (pos1 < buffer.size() && pos2 < buffer.size() && condition(buffer[pos1], buffer[pos2])) {
		 ...
		 * @return Child
		 */
		inline size_t internal_get_correct_child(size_t i1, size_t i2) {
		...
		void internal_push() {
			 ...
			 */
			while (pos != 0 && this->internal_swap_on_demand(pos, (temp = (pos >> 1) - (pos % 2 == 0)))) {
				pos = temp;
			}
		}
		...
		void internal_remove() {
			...
			while (buffer.size() > pos + 1 &&
			       this->internal_swap_on_demand((child = this->internal_get_correct_child(pos, pos + 1)), pos >> 1)) {
				pos = (child << 1) + 1;
			}
			if (buffer.size() > pos) { this->internal_swap_on_demand(pos, pos >> 1); }
		}
